<template>
  <ion-modal :is-open="isOpen" @did-dismiss="cancel">
    <ion-header>
      <ion-toolbar>
        <ion-title>{{ editingId ? 'Edit' : 'Add' }} Hero</ion-title>
    ...
    <ion-content>
      <ion-item>
        <ion-input v-model="localTodo.name" label="Name" label-placement="floating"
          placeholder="Enter Name"></ion-input>
      </ion-item>
      <ion-item>
        <ion-textarea v-model="localTodo.story" label="Story" label-placement="floating"
          placeholder="Enter Story" :autogrow="true" :rows="3"></ion-textarea>
      </ion-item>
      <ion-row>
        <ion-col>
          <ion-button type="button" @click="input" shape="round" color="primary" expand="block">
            {{ editingId ? 'Edit' : 'Add' }} Hero
          </ion-button>
        </ion-col>
      </ion-row>
    </ion-content>
...
// update local hero dengan props
