      ...
      <ion-list>
        <ion-item-sliding v-for="todo in activeTodos" :key="todo.id" :ref="(el) => setItemRef(el, todo.id!)">
          ...
          <ion-item>
            <ion-card>
              <ion-card-header>
                <ion-card-title class="ion-text-wrap limited-text">{{ todo.name }}</ion-card-title>
                <ion-card-subtitle class="limited-text">{{ todo.story }}</ion-card-subtitle>
              </ion-card-header>
        ...
        <ion-item v-if="activeTodos.length === 0" class="ion-text-center">
          <ion-label>No active hero</ion-label>
        </ion-item>
      </ion-list>
    </div>
          ...
          <div slot="content" class="scrollable-container">
            <ion-list>
              ...
              <ion-item v-if="completedTodos.length === 0" class="ion-text-center">
                <ion-label>No completed hero</ion-label>
              </ion-item>
            </ion-list>
          </div>
        </ion-accordion>
...
const handleStatus = async (statusTodo: Todo) => {
  ...
  try {
    ...
    await showToast(
      `Hero marked as ${!statusTodo.status ? 'completed' : 'active'}`,
      'success',
      checkmarkCircle
    );
    loadTodos();
